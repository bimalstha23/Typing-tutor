...
int mainmenu(void)
{
	...
	cprintf("\n\n\n\n\n\r%26s%c T Y P I N G  T U T O R %c","",16,17);
	_setcursortype(_NOCURSOR);
	DrawMenu(1,menuitem,6);  /*default select*/
	while((c=getch())!='\r')/*while return key not pressed*/
	{
		if(c==0)     /*if extended key*/
		{
			extended=getch();
			switch(extended)
			{
...
int listUser(void)
{
	...
	if( (fuser = fopen(userfilename,"r+b")) == NULL )
	{
		printf("\n\nInvalid filename...");
		printf("\npress any key to continue...");
		getch();
...
void DrawMenu(int opt,char *mi[],int n)
{
   ...
   int bgcl[2]={BLACK, /*deselection bg color*/
	...
	box(28,8,25,n*2+2,YELLOW,BLACK);
	cputs(mi[n]);
   for(i=0;i<n;i++)
   {
...
void viewRecords(void)
{
	...
	CLRSCR
	printf("\t\t<U S E R  S T A T I S T I C S>");
...
void about1(void)
{
   ...
   window(6,5,80,25);
   textcolor(LIGHTRED);
   cprintf("\n A B O U T   T Y P I N G   T U T O R");
   textcolor(GREEN);
	cprintf("\n\r \xaf  This application can only run on tubro c++ compiler,\
	\n\r  this console  based typing tester measures your typing speed and display the speed and accuracy.\
	\n\r  this console application also includes one game for users  entertainment.");
getch();
}
void help(void){
	CLRSCR
